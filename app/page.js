      ...
      {floatingCards.map((c, i) => {
        ...
        return (
          <motion.div key={c.t} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i*0.18, duration: 0.6 }}
            className={` md:flex absolute ${positions[i]} bg-white shadow-2xl border border-slate-200 rounded-xl p-3 items-center gap-2.5 max-w-[200px] animate-float`}
...
/* ================= NEW: icons drift across the whole hero, any direction ================= */
function FloatingVerificationIcons() {
  const icons = [
    { Icon: Briefcase, color: '#0052CC', bg: 'from-[#EAF2FF] to-white', ring: 'ring-[#0052CC]/10', pos: 'top-[12%] left-[6%]', anim: 'vc-path-1', duration: '13s', delay: '0s' },
    { Icon: GraduationCap, color: '#F97316', bg: 'from-orange-50 to-white', ring: 'ring-orange-100', pos: 'top-[18%] right-[8%]', anim: 'vc-path-2', duration: '15s', delay: '-3s' },
    { Icon: Fingerprint, color: '#0052CC', bg: 'from-[#EAF2FF] to-white', ring: 'ring-[#0052CC]/10', pos: 'bottom-[22%] left-[10%]', anim: 'vc-path-3', duration: '14s', delay: '-6s' },
    { Icon: MapPin, color: '#F97316', bg: 'from-orange-50 to-white', ring: 'ring-orange-100', pos: 'bottom-[14%] right-[12%]', anim: 'vc-path-4', duration: '16s', delay: '-9s' },
  ]

  return (
    <div className=" sm:block absolute inset-0 z-0 pointer-events-none overflow-hidden">
      <style jsx>{`
        @keyframes vc-path-1 {
